package android.example.playpingpong;
...
public class PlayPingPong implements Runnable {
    ...
    private HandlerThread handlerThreadPing;
    private HandlerThread handlerThreadPong;
    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        protected void onLooperPrepared() {
            ...
            if(mMyType== PingPong.PING) {
                ...
                message.obj = handlerPong;
                message.arg1 = mIterationsCompleted;
        ...
        public boolean handleMessage(Message reqMsg) {
            ...
            // appropriate code.
            Handler relayHandler = reqMsg.getTarget();
            Handler targetHandler = (Handler) reqMsg.obj;

            mIterationsCompleted = reqMsg.arg1;
            if (mIterationsCompleted <= mMaxIterations) {
                if(relayHandler.equals(handlerPing)) {
                    mOutputStrategy.print("PING("+mIterationsCompleted+")");
                } else if(relayHandler.equals(handlerPong)){
                ...
                // @@ TODO - you fill in here.
                handlerThreadPing.quit();
                handlerThreadPong.quit();
                return true;
            }
            ...
            // the reply.
            // @@ TODO - you fill in here.
            Message message = relayHandler.obtainMessage(reqMsg.what);
    ...
    public void run() {
        ...
        // @@ TODO - you fill in here.
        handlerThreadPing = new PingPongThread(PingPong.PING);
        handlerThreadPong = new PingPongThread(PingPong.PONG);

        // Start ping and pong threads, which cause their Looper to
        // loop.
        // @@ TODO - you fill in here.
        handlerThreadPing.start();
        handlerThreadPong.start();

        // Barrier synchronization to wait for all work to be done
        // before exiting play().
        // @@ TODO - you fill in here.
        try {
            handlerThreadPing.join();
            handlerThreadPong.join();
